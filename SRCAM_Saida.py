...
def find_closest_matricula(input_str):
    ...
    sql = "SELECT matricula FROM matriculas"
    cursor.execute(sql)
            ...
            if(tempos == 0):
                ...
                try:
                    ...
                    if(texto_formatado == ""):
                        ...
                        matricula = find_closest_matricula(texto_limpo)
                        if(matricula == 0):
                            print("ERRO: Matrícula não está registada na base de dados.")
                        else:
                            #print("foi aqui")
                            sql = "SELECT matricula FROM subscricao WHERE matricula = ""'" + matricula + "'"" ;"
                            cursor.execute (sql)
                            for row in cursor.fetchall():
                                valor = row[0]
                            if(valor != matricula):
                                #print("foi matricula")
                                sql = "SELECT pagou FROM matriculas WHERE matricula = ""'" + matricula + "'"";"
                                cursor.execute (sql)
                                for row in cursor.fetchall():
                                    valor = row[0]
                                if(valor == 0):
                                    ...
                                    if(valor == 1):
                                        #print("foi update")
                                        sql = "UPDATE registromatriculas SET datasaida = ""'" + data_hora_formatada + "'"" WHERE matricula = ""'" + matricula + "'"" ;"
                                        cursor.execute (sql)
                                        conn.commit()
                                ...
                                if(valor == matricula):
                                    #print("foi subscricao")
                                    sql = "SELECT dataentrada FROM matriculas WHERE matricula = ""'" + matricula + "'"" ;"
                                    cursor.execute (sql)
                                    for row in cursor.fetchall():
                                        data_entrada = row[0]
                                    data_formatada = data_entrada.strftime ('%Y-%m-%d %H:%M:%S')
                                    sql = "INSERT INTO registromatriculas (matricula, dataentrada) VALUES(%s, %s);"
                                    params = (matricula, data_formatada)
